...
import os
from prompt_to_prompt.ptp_classes_adaptive_sd_0215 import AttentionStore, AttentionReplace, AttentionRefine, EmptyControl,load_512
...
from ddm_inversion.inversion_adaptive_sd_0215 import  inversion_forward_process, inversion_reverse_process
...
if __name__ == "__main__":
            ...
            for cfg_scale_tar in cfg_scale_tar_list:
                ...
                for skip in skip_zs:    
                    ...
                    if args.mode=="our_inv":
                        ...
                        controller.max_iter = args.max_iter
    ...
    # =====================================================================
    # 모든 이미지 처리 완료 후 정량 평가 자동 실행
    # =====================================================================
    import sys
    _eval_dir = "/data/aivl-chaewon/Image-Editing-Evaluation"
    if _eval_dir not in sys.path:
        sys.path.insert(0, _eval_dir)
    from evaluate import run_evaluation
    from datetime import datetime as _dt

    eval_yaml_path = os.path.abspath(args.dataset_yaml)
    print(f"\n[Eval] 평가 시작 — yaml: {eval_yaml_path}")
    print(f"[Eval] 결과 폴더: {save_path}\n")

    lpips_mean, clip_obj_mean, clip_img_mean = run_evaluation(
        yaml_path=eval_yaml_path,
        res_folder=save_path,
        model_name="ours",
        device=device,
    )
    ...
    _SPREADSHEET_ID   = "1_VE2Kvcg-3Nni-4G7na_6erO1ydoK8DC4XuV2yYxyPw"
    _CREDENTIALS_PATH = "/data/aivl-chaewon/credentials/gen-lang-client-0466232730-6d3c47e16d56.json"
    _SHEET_NAME       = f"Sheet{args.model_id}"  # 기록할 시트 탭 이름

    dataset_name = os.path.splitext(os.path.basename(args.dataset_yaml))[0]
    exec_time    = _dt.now().strftime("%Y%m%d-%H%M%S")
    exec_code    = " ".join(sys.argv)
    input_cond   = (
        f"model_id={args.model_id},max_iter={args.max_iter},"
        f"bce_grad={args.bce_grad},bce_optim={args.bce_optim},bce_neg={args.bce_neg},"
        f"mul_grad={args.mul_grad},mul_optim={args.mul_optim},"
        f"out_mode={args.out_mode}"
    )
    # GPU 정보
    ...
    _HEADERS = [
        "실행시간(년월일-시분초)", "실행코드", "입력조건값", "GPU",
        "결과경로", "LPIPS", "CLIP-object", "CLIP-image",
        "데이터셋yaml경로", "데이터셋이름",
    ]
    _new_row = [
        exec_time, exec_code, input_cond, _gpu_info,
        save_path, lpips_mean, clip_obj_mean, clip_img_mean,
        eval_yaml_path, dataset_name,
    ]

    try:
        _scopes  = ["https://www.googleapis.com/auth/spreadsheets"]
        _creds   = Credentials.from_service_account_file(_CREDENTIALS_PATH, scopes=_scopes)
        _gc      = gspread.authorize(_creds)
        _ws      = _gc.open_by_key(_SPREADSHEET_ID).worksheet(_SHEET_NAME)

        # 헤더가 없으면 첫 행에 삽입
        _first_row = _ws.row_values(1)
        if _first_row != _HEADERS:
            _ws.insert_row(_HEADERS, index=1)

        _ws.append_row(_new_row, value_input_option="USER_ENTERED")
        print(f"\n[Eval] Google Sheets 에 결과가 저장되었습니다.")
        print(f"       https://docs.google.com/spreadsheets/d/{_SPREADSHEET_ID}")
    except Exception as _e:
        print(f"\n[Eval] Google Sheets 저장 실패: {_e}")
        print("[Eval] 로컬 CSV 에 대신 저장합니다.")
        import csv
        _csv_path    = "/data/aivl-chaewon/DDPM_inversion/eval_log.csv"
        ...
        with open(_csv_path, "a", newline="", encoding="utf-8") as _f:
            ...
            _writer.writerow(_new_row)
        print(f"[Eval] 로컬 저장 완료 → {_csv_path}")
