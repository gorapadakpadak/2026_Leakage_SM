...
import matplotlib
matplotlib.use('Agg')  # 헤드리스 환경용 (display 없어도 저장 가능)
import matplotlib.pyplot as plt

log_dir = "./2026_ECCV_AMOE/logger"
os.makedirs(log_dir, exist_ok=True)
log_file = os.path.join(log_dir, "edit_friendly_log.log")
...
class AttentionStore(AttentionControl):
    ...
    def forward(
        ...
        resolution=None, step_dir="./attn_vis/bce_neg_only", save_steps=None,attn_name = None,timestep=None,**kwargs
    ):
        # import pdb; pdb.set_trace()
        if attn_name is None:
            attn_name = "cross" if is_cross else "self"
        ...
        # print(place_in_unet, attn_name , side_len , attn.shape)
         #* 5) sa 16 block
        if side_len==resolution and not attn_name == "self" and self.cut_inv and self.is_prt:
            # self.saved_src_sa_attn.append(attn.clone())
            # import pdb; pdb.set_trace()
            if timestep.item() not in self.saved_src_sa_attn.keys():
                self.saved_src_sa_attn[timestep.item()] = []
            self.saved_src_sa_attn[timestep.item()].append(attn.clone().detach()) # detach로 메모리 절약
            logger.info(f"src self-attn 저장, step {timestep.item()} / {len(self.saved_src_sa_attn[timestep.item()])}개")
       
        #* 5) sa 16 block
        if side_len==resolution and attn_name == "self" and self.is_prt : #and place_in_unet !="down":
            # import pdb; pdb.set_trace()
            #TODO: 여기 바꿈
            if self.saved_up_self_attn is None:
                self.saved_up_self_attn = attn.clone()#.detach()
                self.sa_count = 1
            else :
                self.sa_count += 1
                with torch.no_grad():
                    avg_attn = (self.saved_up_self_attn.detach() * (self.sa_count - 1) + attn.detach()) / self.sa_count
                # gradient는 이번 step attn만 유지
                self.saved_up_self_attn = attn + (avg_attn - attn).detach()        
    

        #* 1) ca 16 block
        if attn_name == "cross" and side_len==resolution and self.is_prt : #and place_in_unet !="down":
            
            #TODO: 여기 바꿈
            if self.saved_16_crs_attn is None:
                self.saved_16_crs_attn = attn.clone()#.detach()
                self.ca_count = 1
            # self.saved_16_crs_attn.append(attn.clone())
            #TODO: 
            else :
                # self.saved_16_crs_attn = self.saved_16_crs_attn + attn.clone()#.detach()
                self.ca_count += 1
                with torch.no_grad():
                    avg_attn = (self.saved_16_crs_attn.detach() * (self.ca_count - 1) + attn.detach()) / self.ca_count
                # gradient는 이번 step attn만 유지
                self.saved_16_crs_attn = attn + (avg_attn - attn).detach()
            
        # ---------------------- vis_steps에 해당될 때만 파일로 저장 ---------------------- #
        if getattr(self, "vis_mode", False) and timestep is not None and self.is_rev and side_len==resolution:
            # if self.is_rev :
            #     import pdb; pdb.set_trace()
    ...
    # =================== DEBUG VISUALIZATION =================== #
    def debug_vis_mask_attn(self, attn_prob_b, mask_tensor, tok_idx_group, side_len,
                             save_dir="./debug_vis", tag=""):
        """
        마스크가 attention에 어떻게 씌워지는지 시각화.

        Args:
            attn_prob_b : [HW, T]  – 배치 평균 cross-attention map
            mask_tensor : [HW]     – 이진 마스크 (0/1)
            tok_idx_group : list   – 확인할 토큰 인덱스 목록
            side_len : int         – sqrt(HW), 보통 16
            save_dir : str         – 저장 경로
            tag : str              – 파일명 접두사

        pdb에서 호출법:
            self.debug_vis_mask_attn(attn_prob_b, mask_tensor, token_indices, side_len, tag="step0")
        """
        os.makedirs(save_dir, exist_ok=True)
        mask_2d = mask_tensor.detach().cpu().float().reshape(side_len, side_len).numpy()
        attn_np = attn_prob_b.detach().cpu().float().numpy()   # [HW, T]

        n_tokens = len(tok_idx_group)
        n_cols = 3  # mask | full attn | masked attn
        n_rows = max(n_tokens, 1)

        fig, axes = plt.subplots(n_rows, n_cols, figsize=(n_cols * 3, n_rows * 3))
        if n_rows == 1:
            axes = axes[np.newaxis, :]   # 항상 2D로

        col_titles = ["Mask", "Attn (full)", "Attn × Mask"]
        for c, title in enumerate(col_titles):
            axes[0, c].set_title(title, fontsize=10)

        for row, t in enumerate(tok_idx_group):
            attn_map = attn_np[:, t].reshape(side_len, side_len)  # [H, W]
            masked_attn = attn_map * mask_2d

            # --- mask ---
            im0 = axes[row, 0].imshow(mask_2d, cmap="gray", vmin=0, vmax=1)
            axes[row, 0].set_ylabel(f"token {t}", fontsize=8)
            axes[row, 0].axis("off")
            plt.colorbar(im0, ax=axes[row, 0], fraction=0.046)

            # --- full attention ---
            im1 = axes[row, 1].imshow(attn_map, cmap="hot", vmin=0)
            axes[row, 1].axis("off")
            plt.colorbar(im1, ax=axes[row, 1], fraction=0.046)

            # --- masked attention ---
            im2 = axes[row, 2].imshow(masked_attn, cmap="hot", vmin=0)
            axes[row, 2].axis("off")
            plt.colorbar(im2, ax=axes[row, 2], fraction=0.046)

            # mask contour overlay on attn
            axes[row, 1].contour(mask_2d, levels=[0.5], colors="cyan", linewidths=1.5)
            axes[row, 2].contour(mask_2d, levels=[0.5], colors="cyan", linewidths=1.5)

            # 통계 출력
            in_sum  = float(masked_attn.sum())
            out_sum = float((attn_map * (1 - mask_2d)).sum())
            ratio   = in_sum / (in_sum + out_sum + 1e-12)
            print(f"  [token {t}] in_attn={in_sum:.4f}  out_attn={out_sum:.4f}  ratio={ratio:.4f}")

        fig.suptitle(f"Mask × Attention  |  side={side_len}  {tag}", fontsize=11)
        plt.tight_layout()

        save_path = os.path.join(save_dir, f"{tag}_mask_attn_vis.png")
        plt.savefig(save_path, dpi=120, bbox_inches="tight")
        plt.close(fig)
        print(f"[debug_vis] saved → {save_path}")
        return save_path
    # =========================================================== #
    ...
    def compute_ca_bce_loss(self, xt,mask_token_pairs, timestep):
        ...
        for attn_map in [self.saved_16_crs_attn]:  
            ...
            attn_prob_b = attn_map.mean(dim=0)
            for i, (tok_idx_group, mask_tensor, obj_tensor) in enumerate(zip(target_token_indices, mask_tensors, obj_tensors)):
                ...
                # ── DEBUG: 마스크 × attention 시각화 ─────────────────────────
                # pdb에서 다음 명령 실행:
                #   self.debug_vis_mask_attn(attn_prob_b, mask_tensor, tok_idx_group, side_len, tag=f"step{int(timestep.item())}_pair{i}")
                # 자동 저장 후 "./debug_vis/" 폴더 확인
                # import pdb; pdb.set_trace()
                # ─────────────────────────────────────────────────────────────
                ...
                #TODO: bce negative adaptive weight - - - - - - - #
                ca_per_token = ca_per_token / len(token_indices)
    ...
    def compute_region_loss_with_adaptive_scale(self, attn_prob_b, mask, bg_mask_b, obj_mask, 
        ...
        for t in tok_idx_group:
            ...
            if out_mode in ['bg_bg','bg+sem_bg','bg+inst_bg']:
                ...
                instance_region_sum = ca_per_token[obj_mask==1].sum()
    ...
    def prepare_masks(self, mask_token_pairs, side_len, HW):
    ...
    def __init__(self):
        ...
        self.step_store = self.get_empty_store()
        self.attention_store = {}
        self.cur_step = 0  # current timestep
        self.region_cur_step_data=[]
        self.region_step_logs = []  # 전체 스텝 로그
        self.img_num = 0
        self.sa_count =1
        # self.region_loss_for_grad = None
        self.cut_mode = False
        ...
        #TODO:
        self.saved_up_self_attn = None
        self.saved_16_crs_attn = None
        ...
        self.num_tokens = 77
        self.vis_mode = False
        self.vis_steps = [631,581,531,481,431,421,331,231,1]
        self.save_path_vis = './SDXL_ouputs/attention_vis'
        self.iteration = 0
