...
class AttentionStore(AttentionControl):
    ...
    def forward(
        ...
        #* 5) sa 16 block
        # if side_len==16 and not is_cross and attn_name == "attn1" and self.is_rev and self.is_prt:
        if side_len==16 and attn_name == "self" and self.is_rev :
        ...
        #* 1) ca 16 block
        # if is_cross and attn_name == "attn2" and side_len==16 and self.is_rev and self.is_prt: #* 260218
        if attn_name == "cross" and side_len==16 and self.is_rev :
        #* 2) ca up 16 block
    ...
    def compute_ca_bce_loss(self, xt,mask_token_pairs, timestep):
        ...
        for attn_map in self.saved_16_crs_attn:  
            ...
            for i, (tok_idx_group, mask_tensor, obj_tensor) in enumerate(zip(target_token_indices, mask_tensors, obj_tensors)):
                # import pdb; pdb.set_trace()
                for b in range(B):
                    attn_prob_b = attn_map[b]
                    
                    mask_tensor_b = mask_tensor[b]#.mean(dim=0)
                    region_attn = attn_prob_b[mask_tensor_b == 1, :]  # [#pix, T]
